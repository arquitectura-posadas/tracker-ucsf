...
let estado = JSON.parse(localStorage.getItem("ucsf_tracker_v2")) || {};
...
function render() {
    ...
    if (!listCursar.innerHTML) listCursar.innerHTML = "<li>No hay materias habilitadas.</li>";
    if (!listRendir.innerHTML) listRendir.innerHTML = "<li>No tenés materias regulares por rendir.</li>";
...
function openModal(m) {
    ...
    const reqText = m.cReg ? `Requiere: ${m.cReg.join(', ')}` : 'Sin correlativas.';
...
function setMateriaEstado(s) {
    ...
    localStorage.setItem("ucsf_tracker_v2", JSON.stringify(estado));
    closeModal();
    render();
}
